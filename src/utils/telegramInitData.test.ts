...
describe('single source of init data', () => {
  it('is read through this module only', async () => {
    ...
    // fileURLToPath (not raw .pathname) so this resolves correctly on
    // Windows too — a bare pathname keeps the leading '/' before the drive
    // letter, which path.join then turns into a bogus '\C:\...' root.
